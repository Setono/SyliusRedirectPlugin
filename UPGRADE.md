...
| Route `setono_sylius_redirect_admin_redirect_check_source` (path `/admin/redirects/check-source`) | Renamed to `setono_sylius_redirect_admin_ajax_redirect_check_source` and moved under the `/ajax` admin sub-prefix (path `/admin/ajax/redirects/check-source`), matching Sylius's own AJAX route convention. If you generated the URL by name in your own templates or JS, update the name; if you hand-coded the path, update it too. The plugin's bundled JS now also sends the `X-Requested-With: XMLHttpRequest` header on this request. |
| All XML files under `config/services/` (and `config/services.xml`)                       | Replaced by PHP DSL equivalents (`config/services.php` + `config/services/*.php`) loaded via `PhpFileLoader`. Same service ids, arguments, tags, decorators, and aliases. Only relevant if you imported the plugin's XML files directly or copied them into your own bundle layout. |
| `Setono\SyliusRedirectPlugin\Menu\AdminMenuListener` (service id `setono_sylius_redirect_plugin.menu.admin_menu`) | Replaced by `Setono\SyliusRedirectPlugin\EventSubscriber\AdminMenuSubscriber` (registered with the `kernel.event_subscriber` tag, service id is the FQCN). The `Menu/` namespace is gone. If you decorated or fetched the listener from the container, update the class name and the service id. |
| Service ids `setono_sylius_redirect.event_subscriber.request` and `setono_sylius_redirect.event_listener.not_found` | Renamed to their FQCNs (`Setono\SyliusRedirectPlugin\EventSubscriber\RequestSubscriber` and `Setono\SyliusRedirectPlugin\EventSubscriber\NotFoundSubscriber`). Only relevant if you fetched, decorated, or aliased these via the snake-cased ids. |
| Snake-cased service ids `setono_sylius_redirect.command.prune`, `setono_sylius_redirect.custom_factory.redirect`, `setono_sylius_redirect.finder.removable_redirect`, `setono_sylius_redirect.form.type.redirect`, `setono_sylius_redirect.pruner.redirect`, `setono_sylius_redirect.resolver.redirection_path`, `setono_sylius_redirect.twig.extension`, `setono_sylius_redirect.twig.runtime`, `setono_sylius_redirect.url_resolver.automatic_redirect.composite`, `setono_sylius_redirect.url_resolver.automatic_redirect.product`, and `setono_sylius_redirect.url_resolver.automatic_redirect.taxon` | All renamed to the FQCN of their concrete class (e.g. `Setono\SyliusRedirectPlugin\Pruner\Pruner`, `Setono\SyliusRedirectPlugin\Resolver\RedirectionPathResolver`). The corresponding `*Interface` services keep working as aliases pointing at the new FQCN ids — fetch by the interface (`$container->get(RedirectionPathResolverInterface::class)`) for forward-compatible code. The Sylius-resource-bundle-managed ids (`setono_sylius_redirect.factory.redirect`, `repository.redirect`, `manager.redirect`) are unchanged because they are auto-generated from the resource config. |
| `Setono\SyliusRedirectPlugin\EventSubscriber\RedirectResponseTrait`                      | Replaced by the new `Setono\SyliusRedirectPlugin\EventSubscriber\AbstractRedirectSubscriber` base class. Both `RequestSubscriber` and `NotFoundSubscriber` now extend it; the trait's `getRedirectResponse()` helper is no longer publicly available. If you `use`-d the trait in your own subscriber, extend the abstract class instead. |
| `Setono\SyliusRedirectPlugin\Model\RedirectInterface::onAccess()`                        | Renamed to `markAsAccessed()` (same behaviour: increments `count`, stamps `lastAccessed = now`). If you implement `RedirectInterface` directly or call the method by name, update both the declaration and the call sites. |
