        ...
        $rootNode
            ...
            ->children()
                ->integerNode('remove_after')
                    ...
                    ->defaultValue(0)
                ->end()
                ->booleanNode('allow_non_404_redirects')
                    ->info('When true (default), the plugin checks for a matching redirect on every request. Set to false if you only use only-404 redirects to skip the per-request database query.')
                    ->defaultTrue()
                ->end()
